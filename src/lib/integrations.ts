...
export const handleDropboxAuth = async (): Promise<{ accessToken: string; refreshToken: string, expiresIn: number }> => {
    ...
    return new Promise<{ accessToken: string; refreshToken: string, expiresIn: number }>((resolve, reject) => {
...
async function exchangeAuthorizationCode(code: string, clientId: string, clientSecret: string) {
