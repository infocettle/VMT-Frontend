      ...
      <div key={modules.id} className="flex items-center justify-between w-full">
        {modules.children && modules.children.length > 0 ? (
            ...
            <DropdownMenuTrigger asChild>
              <div className="w-full flex gap-2 items-center justify-between">
                <div className="cursor-pointer">
                  <h3 className="page_details_mobile">
                    {modules.name}
                  </h3>
                </div>
                {modules.icon ? (
              ...
              <div className="flex flex-col items-start">
                {modules.children.map((child) => (
                  <Link key={child.id} to={child.href}>
                    <DropdownMenuItem key={child.id} className="capitalize text-xs text-black font-light">
                      {child.name}
                    </DropdownMenuItem>
                  </Link>
                ))}
          ...
          <Link className="self-start cursor-pointer" to={modules.href}>
            <h3 className={`text-vmtpurple focus-within:text-vmtblue text-xs leading-relaxed`}>
              {modules.name}
            </h3>
          </Link>
        )}
