...
const CancelDomain = () => {
  ...
  const { data: allSubscribers, isPendingSubscribers } = useFetchData(
    cancelDomainUrl,
    "cancelDomain"
  );
  ...
  const {
    ...
    isPending: isPendingIndividualSubscribers,
  } = useFetchData(individualDomainUrl);
      ...
      <div className="bg-gray-100 py-3 px-10 w-full flex-col items-center">
        <div className="flex justify-between w-full items-center pt-5">
          ...
          <div className="flex items-center w-auto px-2 space-x-4">
            {cancelDomainVisible && (
              <ConfirmDomain
                open={cancelDomainVisible}
                data={individualSubscriber}
                defaultValue={currentSubscriberID}
                onClose={() => setCancelDomainVisible()}
                onLock={onLock}
                cancel={true}
              />
            )}
